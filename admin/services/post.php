...
class PostFactory {
	...
	public static function GetRelatedPosts($postid)
	{
		...
		$select_sql = "SELECT post.id FROM pbpost post where id <> $postid and post.publishdate is not null and (post.unpublishdate is null or post.publishdate > post.unpublishdate) order by post.modifieddate desc";
		if ($result = $GLOBALS['mysqli']->query($select_sql)) {
			while ($row = $result->fetch_assoc()) {
	...
	{
		$retval = array();
		$select_sql = "SELECT post.id FROM pbpost post where post.publishdate is not null and (post.unpublishdate is null or post.publishdate > post.unpublishdate) order by post.modifieddate desc";
		if ($result = $GLOBALS['mysqli']->query($select_sql)) {
			while ($row = $result->fetch_assoc()) {
				$post = new Post($row['id']);
				if ($post->CanPublish) {
	...
	//get all blogs that can be published and are ready for publish
	public static function GetAllPublishableBlogs()
	{
...
class Post {
	...
	public function Save()
	{
		if ($this->ID < 1) {	//add new
			$insert_sql = function() {
				...
				if (!empty($this->Title)) {
					$col_list .= 'title,';
					$val_list .= Post::escapebackslash("'$this->Title',");
				}
				if (!empty($this->SubTitle)) {
					$col_list .= 'subtitle,';
					$val_list .= Post::escapebackslash("'$this->SubTitle',");
				}
				if (!empty($this->Blog)) {
					$col_list .= 'blog,';
					$val_list .= Post::escapebackslash("'$this->Blog',");
				}
				if (!empty($this->PageName)) {
					$col_list .= 'pagename,';
					$val_list .= Post::escapebackslash("'$this->PageName',");
				}
			...
			$update_sql = function() {
				...
				$retval .= empty($this->Title) ? "title = null, " : "title = '" . Post::escapebackslash($this->Title) . "', ";
				$retval .= empty($this->SubTitle) ? "subtitle = null, " : "subtitle = '" . Post::escapebackslash($this->SubTitle) . "', ";
				$retval .= empty($this->Blog) ? "blog = null, " : "blog = '" . Post::escapebackslash($this->Blog) . "', ";
				$retval .= empty($this->PageName) ? "pagename = null, " : "pagename = '" . Post::escapebackslash($this->PageName) . "', ";
	...
	private static function escapebackslash($string) {
	...
	public function NewResponse()
	{
		...
		//comment (text)
		//curated = TRUE / FALSE
		//TODO: shouldn't this come through the factory
		//and hey how about the use of singleton objects
		//basically we want people to get an object only via it's factory
	...
	{
		//this should do NOTHING if user has not updated the post since last update
		//OR
		...
		if ($this->ModifiedDate > $this->PublishDate || $this->ReadyForPublish) {
		  file_put_contents("blum",1);
			...
			$this->SetReadyForPublish('false');
		}
		else {
			//glogg('not publi');
		}
	}
	private static function setupforbootstrap($blogPost)
	{
		$bootstrapped = '';
		if (!$bootstrapped = preg_replace ('/\<table.+?border=\"\d+?\".+?\>/',"<table class='table table-bordered'>",$blogPost)) {
			$bootstrapped = preg_replace ('/\<table.+?\>/',"<table class='table'>",$blogPost);
		}
		if ($bootstrapped = preg_replace ('/\<a href=\"https\:\/\/www.youtube.com\/watch\?v=(.+?)\".+?\<\/a\>/',"<div class='embed-responsive embed-responsive-4by3'> <iframe class='embed-responsive-item' src='//www.youtube.com/embed/$1' frameborder=0 allowfullscreen></iframe></div>",$bootstrapped)) {
		}
		// $bootstrapped = preg_replace ('/\<a href=\"https\:\/\/www\.youtube\.com\/watch\/.+?)\".*?\>.+?\<\/a\>/',,$bootstrapped);
		return $bootstrapped;
	}
	public static function setpagename($blogPost)
	{
		...
		//just to make sure, let's remove the .html or .htm from the end of the page name before we clean up
		$blogPost->PageName = rtrim($blogPost->PageName,'.htm(l)');
		//make sure the url is nice
		$blogPost->PageName = preg_replace('/[\s\`\~\!\@\#\$\%\^\&\*\(\)\+\=\{\[\}\]\:\;\'\"\,\<\.\>\/\?]/','_',$blogPost->PageName);
		
		
		/*
		we need to figure out a good way to handle duplicate names
		
		$file_inc = 1;
		$filename = $blogPost->PostFolder . $blogPost->PageName;
			logg('here' . $filename . 'html', FILE_APPEND);
		while (file_exists($filename . '.html')) {
			$file_inc++;
			$filename = $filename .  $file_inc;
		}
		logg($filename,FILE_APPEND);
		*/
		
		
		//and then let's add it back
	...
	//these are explicit function to update stuff like publish date and ready-for-publish because these are not really post updates
	private function updatePublishDate()
	{
		$update_sql = "update pbpost set publishdate = now() where id = '" . $this->ID . "';";
		$GLOBALS['mysqli']->query($update_sql);
	}
	
	public function SetReadyForPublish($state)
	{
		$update_sql = "update pbpost set readyforpublish = ";
		if ($state === 'false') {
			$update_sql .= "null";
		}
		else {
